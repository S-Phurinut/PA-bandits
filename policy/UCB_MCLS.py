...
class UCB_Monotone_Concave_LeastSquare():
        ...
        #-------Phase2: Play Bandit Game-------
            ...
            if self.type_arm=='participation-based':
                ...
                if np.sum(self.num_reward<1)>0:
                    ...
                    est_fun=lambda x : f.value[x]+np.sqrt(2*np.log(info['curr_round'])/self.num_reward[x-1])
                    # UCB=np.zeros((self.player.num_agent+1,))
                    # for arm in range(0,self.player.num_agent):
                    #     if self.num_reward[arm]==0:
                    #         UCB[arm+1]=1
                    #     else:
                    #         UCB[arm+1]=self.sum_reward[arm]/self.num_reward[arm]+np.sqrt(2*np.log(info['curr_round'])/self.num_reward[arm])
                    #         # UCB[arm+1]=self.sum_reward[arm]/self.num_reward[arm]+2*np.sqrt(np.log(info['max_round'])/self.num_reward[arm])
                    # UCB=np.min((UCB,np.ones((self.player.num_agent+1,))),axis=0)
                    # print("UCB value=",UCB)
                    print("num reward=",self.num_reward)
                
                    # obj = cp.Minimize(cp.sum_squares(UCB - f))
                    # prob = cp.Problem(obj, cons)
                    # prob.solve(solver=cp.OSQP)
