...
class BayesUCB_Gibbs_Monotone():
    def __init__(self,type_arm,num_cost_learning=1,cost_alg='MultiBinSearch',num_burnin_sweeps=5, 
                 num_sweeps=100,random_scan=True,eps=1e-12,init_sweep_appr='once',**bandit_alg):
        ...
        #-------Phase2: Play Bandit Game-------
            ...
            if self.type_arm=='participation-based':
                ...
                if np.sum(self.num_reward<self.bandit_alg['min_sample_required'])>0:
                    ...
                    UCB=np.quantile(self.posterior,q=q, axis=0)

                    if info['curr_round']%500==0:   print("BayesUCB=",UCB)

                    #---------------Greedy alg-----------------
                    best_utility=-math.inf
                    pulled_arm=0
                    for arm in range(self.player.num_agent):
                        utility=UCB[arm]-self.cum_cost[arm]
