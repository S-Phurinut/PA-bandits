...
class Bayesian_Bernoulli_Regression_TS():
        ...
        #-------Phase2: Play Bandit Game-------
            ...
            if self.type_arm=='participation-based':
                ...
                if np.sum(self.num_reward<=0)>0:
                    ...
                    # w_samples = trace.posterior["w"].stack(draws=("chain","draw")).values  # shape: (K-1, n_samples)

                    # n_samples = w_samples.shape[1]
                    # # Thompson Sampling: sample one posterior draw
                    # idx = np.random.randint(0, n_samples)
                    # w_thompson = w_samples[:, idx]
                    # # Compute latent function f_k for arms 1..K-1
                    # f_all = np.dot(self.X, w_thompson)
                    # f_all[0] = 0.0  # first arm probability = 0
                    # print(f_all)
                    # print(f_all.shape)
                    # # Compute hazard-style probabilities
                    # p_all = 1 - np.exp(-f_all)

                    self.p_samples = self.trace.posterior["p_all"].stack(draws=("chain","draw")).values
                    p_all=self.p_samples[:,np.random.randint(0,self.p_samples.shape[1])][1:] #exclude posterior of 0 agents
                    print("avg reward=",self.sum_reward/self.num_reward)
                    print("posterior mean=",self.p_samples.mean(axis=1)[1:])
                    print("posterior std=",self.p_samples.std(axis=1)[1:])
                    print("TS success prob=",p_all)
                    print("TS net reward=",p_all-self.cum_cost)
                ...
                if self.is_model_fitted==False:
                    ...
                    best_arm=np.argmax(self.p_samples.mean(axis=1)[1:]-self.cum_cost)
                self.best_incentive=np.zeros((self.N,))
