...
class TS_GDD(): #the Stick-breaking construction or the Generalized Dirichlet Distribution
                ...
                if np.sum(self.num_reward<self.bandit_alg['min_sample_required'])>0:
                    ...
                    if refit_model:
                        with pm.Model() as model:
                            # 1. Hyper-priors for local jump characteristics
                            # We use a Gamma prior for the a and b parameters of each stick-breaking step.
                            # This allows the model to learn if a specific arm should have a large or small jump.
                            alpha_hyper = pm.Gamma("alpha_hyper", alpha=2.0, beta=1.0, shape=self.player.num_agent)
                            beta_hyper = pm.Gamma("beta_hyper", alpha=2.0, beta=1.0, shape=self.player.num_agent)

                            # 2. Latent stick-breaking proportions (z_i)
                            # Each z_i is the proportion of the *remaining* probability space taken by arm i.
                            z = pm.Beta("z", alpha=alpha_hyper, beta=beta_hyper, shape=self.player.num_agent) 
                            # 3. Construct the increasing function f
                            # f_k = 1 - product_{i=1 to k} (1 - z_i)
                            # This is the "General" form that satisfies 0 <= f_1 <= ... <= f_n <= 1
                            one_minus_z = 1.0 - z
                            failure_prob = pm.math.cumprod(one_minus_z) 
                            f = pm.Deterministic("f", 1.0 - failure_prob)
