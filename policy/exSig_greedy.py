...
class ExtendedSigmoid_greedy():
    ...
    def run(self,**info):
        ...
        #-------Phase2: Play Bandit Game-------
            ...
            if self.type_arm=='participation-based':
                ...
                optimise=sc.optimize.minimize(self.extended_sigmoid_loss_function,x0=self.x0,bounds=[(0,math.inf),(0,math.inf)])
                para=optimise.x
                print("est_para=",para)
                self.x0=optimise.x

                #---------------Greedy alg-----------------
                exSig=lambda x : (1-np.exp(-np.sqrt(x)*para[0]-x*para[1]))/(1+np.exp(-np.sqrt(x)*para[0]-x*para[1]))
                best_utility=-math.inf
                pulled_arm=0
                for arm in range(self.player.num_agent):
                    utility=exSig(arm+1)-self.cum_cost[arm]
    ...
    def extended_sigmoid_loss_function(self,para):
        sum_loss=0
        for id_data in range(len(self.Y)):
            loss=(self.Y[id_data]-(1-np.exp(-np.sqrt(self.X[id_data])*para[0]-self.X[id_data]*para[1]))/(1+np.exp(-np.sqrt(self.X[id_data])*para[0]-self.X[id_data]*para[1])))**2
            sum_loss+=loss
        return sum_loss
