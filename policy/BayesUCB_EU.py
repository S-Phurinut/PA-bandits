...
class BayesUCB_EU(): #EU with agent approx model
    ...
    def run(self,**info):
        ...
        if info['curr_round']<=self.num_cost_learning:
            ...
            if self.type_arm=='participation-based':
                ...
                if self.alg['model_conf_bound']=='1/T':
                    self.q = 1.0 - 1.0 / max(2, info['curr_round'])
    ...
    def opt_EU_value(self, cost, reward):
    ...
    def EU_value(self, cost, reward):
        ...
        if np.sum(p_reward <= 1E-4) >= self.player.num_agent:
            EU += 0.0
        elif np.sum(p_reward >= 1 - 1E-12) >= self.player.num_agent:
            EU += reward[self.player.num_agent - 1] - np.dot(p_pay, cost)
        else:
            pb = PoiBin(p_reward)
            num_offered_agent = int(np.sum(p_reward > 1E-6))
            num_guaranteed_agent = int(np.sum(p_reward >= 1 - 1E-6))
            for arm in range(max(num_guaranteed_agent, 1), num_offered_agent + 1):
                EU += reward[arm - 1] * np.clip(pb.pmf(arm), 0, 1)
