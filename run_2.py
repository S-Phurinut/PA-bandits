...
@hydra.main(version_base=None,config_path="config", config_name="main3")
def main(config):
    ...
    M=config.num_sim
    T=config.max_round
    N=config.setting.num_agent

    regret_array=np.zeros((M,T))
    simple_regret_array=np.zeros((M,T))
    num_agent_response_array=np.zeros((M,T))
    num_agent_response_regret_array=np.zeros((M,T))
    ...
    try:
        ...
        mean_num_agent_response_array=np.mean(num_agent_response_array,axis=0)
        mean_num_agent_response_regret_array=np.mean(num_agent_response_regret_array,axis=0)
        sum_num_agent_response_regret_array=np.sum(num_agent_response_regret_array,axis=0)
        sqsum_num_agent_response_regret_array=np.sum(num_agent_response_regret_array**2,axis=0)

        num_agent_response_l1_regret_array=np.abs(num_agent_response_regret_array)
        ...
        for t in range(T):
            ...
            wandb.log({"round":t,"mean_num_agent_response":mean_num_agent_response_array[t],
                    "mean_num_agent_response_regret":mean_num_agent_response_regret_array[t],"sum_num_agent_response_regret":sum_num_agent_response_regret_array[t],"sqsum_num_agent_response_regret":sqsum_num_agent_response_regret_array[t],
                    "mean_num_agent_response_l1_regret":mean_num_agent_response_l1_regret_array[t],"sum_num_agent_response_l1_regret":sum_num_agent_response_l1_regret_array[t],"sqsum_num_agent_response_l1_regret":sqsum_num_agent_response_l1_regret_array[t],
